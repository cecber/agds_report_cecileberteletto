...
#### comments about the clustering (question 2.3.2):
...
```{r}
# environmental clustering function (simple one with juat 2 variables)
environmental_clustering <- function(data, k = 5) {
  set.seed(42)
  ...
  kmeans_result <- kmeans(env_vars_scaled, centers = k, nstart = 25)
  
  data_clean <- data %>% 
    drop_na(map, mat) %>%
    mutate(env_cluster = kmeans_result$cluster)
  
  return(data_clean)
}

# Plot environmental clusters
plot_environmental_clusters <- function(data) {
  
  p <- ggplot(data, aes(x = mat, y = map, color = factor(env_cluster))) +
    geom_point(alpha = 0.6, size = 1) +
    scale_color_brewer(palette = "Set1", name = "Cluster") +
    labs(x = "Mean Annual Temperature (°C)", 
         y = "Mean Annual Precipitation (mm)",
         title = "Environmental Clusters (K-means, k=5)") +
    theme_bw() +
    theme(legend.position = "bottom")
...
# Environmental cross-validation function
environmental_cv <- function(data, k = 5) {
  
  # Add environmental clusters
  data_clustered <- environmental_clustering(data, k = k)
  
  # Prepare data
  data_clean <- data_clustered %>%
    select(leafN, elv, mat, map, ndep, mai, Species, env_cluster) %>%
    drop_na()
  
  # Create fold indices (same logic as spatial CV)
  group_folds_train <- map(
    seq(length(unique(data_clean$env_cluster))),
    ~ {
      data_clean %>%
        select(env_cluster) %>%
        mutate(idx = row_number()) %>%
        filter(env_cluster != .x) %>%
        pull(idx)
    }
  )
  ...
  train_test_by_fold <- function(idx_train, idx_test) {
    ...
    # Prepare predictors and target
    X_train <- train_data %>% select(-leafN, -env_cluster)
    y_train <- train_data$leafN
    X_test <- test_data %>% select(-leafN, -env_cluster)
    y_test <- test_data$leafN
